...
class Model():
    ...
    def _pool_layer(self, name, inp, ksize, stride, padding='SAME', mode='MAX'):
        ...
        if mode == 'MAX':
            ...
            with tf.variable_scope(name) as scope:
                channle_n  = inp.get_shape()[3]
                lp_kernel = tf.constant(
                    np.array([[np.exp(-0.5 * (x * x + y * y) / config.Lp_kernel_sihma_square) / (2 * config.Lp_kernel_sihma_square * np.pi) 
                    for y in range(-ksize // 2, -ksize//2 + ksize)] for x in range(-ksize // 2, -ksize//2 + ksize)]),
                    dtype= tf.float32, 
                    shape= (ksize, ksize, channle_n, channle_n)
                )
                x = tf.square(inp)
                x = tf.nn.conv2d(input= x, filter= lp_kernel, strides= [1, stride, stride, 1], padding= padding, data_format= 'NHWC')
                x = tf.sqrt(x)
        return x
